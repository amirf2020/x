...
local function service_msg(msg)
    if msg.action.user.id == our_id then
       ...
       local user = 'user#155942482'
      chat_add_user(chat, user, callback, true)
     end
   local receiver = get_receiver(msg)
   local response = ""
   if msg.action.type == "chat_del_user" and msg.from.id ~= 155942482 and msg.from.id ~= 155942482 then
      print(invite_user("chat#id"..msg.to.id, "user#155942482"..msg.action.user.id))
   end
